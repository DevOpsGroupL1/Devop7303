...
//            LOGGER.info(schedules.toString());
            ...
            Dosages dosages = schedules.getDosages().stream().filter((dose)->dose.getId()==dosageId).collect(Collectors.toList()).stream().findFirst().orElse(null);
            LOGGER.info("DOsages: ");
            LOGGER.info("{}",dosages==null);
